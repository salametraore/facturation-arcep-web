...
@Component({
  selector: 'autorisations-postales-table',
  templateUrl: './autorisations-postales-table.component.html'
})
export class AutorisationsPostalesTableComponent implements OnInit, AfterViewInit {
    ...
    this.ficheTechniquesService.getFicheTechniques().subscribe((lignes: FicheTechniques[]) => {
      ...
      this.ficheTechniques.data = lignes
        .filter(f =>
          ...
          f.produits_detail?.some(p => p && this.productAllowedIds.includes(p.produit))
        )
        .sort((a, b) => b.id - a.id);
